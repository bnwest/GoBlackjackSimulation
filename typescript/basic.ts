import {CardRank, Card, getCardValue, getCardIndex} from './card';
...
function convertToPlayerDecision(
    ...
    const isFirstPostSplitDecision: boolean =
        isFirstDecision && playerHand.isFromSplit;

    let playerDecision: PlayerDecision = PlayerDecision.STAND;
    ...
    if (decision === S) {
        ...
        let nondoubleDownDecision: PlayerDecision;
        if (decision === Dh) {
        ...
        if (doubleDownAllowed) {
            ...
            playerDecision = doubleDown
                ? PlayerDecision.DOUBLE
                : nondoubleDownDecision;
        } else {
        ...
        let nonsurrenderDecision: PlayerDecision;
        switch (decision) {
            case Uh:
            ...
            default:
                throw new Error(
                    'convertToPlayerDecision() ran into a little trouble in town',
                );
        }
        const surrenderAllowed: boolean =
            isFirstDecision &&
            !isFirstPostSplitDecision &&
            HouseRules.SURRENDER_ALLOWED;
        playerDecision = surrenderAllowed
            ? PlayerDecision.SURRENDER
            : nonsurrenderDecision;
    }
...
function createPairsDecison(): Map<CardRank, Map<CardRank, string>> {
    ...
    */
    const decisions: Map<CardRank, Map<CardRank, string>> = new Map<
        CardRank,
        Map<CardRank, string>
    >();
    ...
    ranks.forEach(playerPairRank => {
        const playerIndex: number = getCardIndex(playerPairRank);
        ranks.forEach(dealerTopCardRank => {
            const dealerIndex: number = getCardIndex(dealerTopCardRank);
            const decision: string = _PAIRS_DECISION[playerIndex][dealerIndex];
...
function createHardTotalDecision(): Map<CardRank, string>[] {
    ...
    */
    const decisions: Map<CardRank, string>[] = [];
    ...
    for (let hardCount = 0; hardCount <= 21; hardCount++) {
        ranks.forEach(dealerTopCardRank => {
            ...
            const decision: string =
                _HARD_TOTAL_DECISION[hardCount][dealerIndex];
    ...
    */
    ...
    for (let hardCount = 0; hardCount <= 21; hardCount++) {
        ranks.forEach(dealerTopCardRank => {
            const dealerIndex: number = getCardIndex(dealerTopCardRank);
            const decision: string =
                _SOFT_TOTAL_DECISION[hardCount][dealerIndex];
            decisions[hardCount].set(dealerTopCardRank, decision);
        });
    }
...
export function determineBasicStrategyPlay(
    ...
    const playerCard1: Card = playerHand.getCard(0);
    const playerCard2: Card = playerHand.getCard(1);

    let decision: string;
    let playerDecision: PlayerDecision;

    let gotPairs: boolean;
    if (isFirstDecision) {
        if (HouseRules.SPLIT_ON_VALUE_MATCH) {
            gotPairs =
                getCardValue(playerCard1.rank) ===
                getCardValue(playerCard2.rank);
        } else {
    ...
    if (gotPairs && handsAllowMoreSplits) {
        ...
        decision = PAIRS_DECISION.get(pairRank)?.get(
            dealerTopCard.rank,
        ) as string;
        if (!isDecisionValid(decision)) {
            throw new Error(
                'determineBasicStrategyPlay() PAIRS_DECISION returns a bad decision ' +
                    decision,
            );
        }
    ...
    const hardCount: number = playerHand.hardCount;
    const softCount: number = playerHand.softCount;
    const useSoftTotal: boolean = hardCount < softCount && softCount <= 21;
    if (useSoftTotal) {
        decision = SOFT_TOTAL_DECISION[softCount].get(
            dealerTopCard.rank,
        ) as string;
        if (!isDecisionValid(decision)) {
            throw new Error(
                'determineBasicStrategyPlay() SOFT_TOTAL_DECISION returns a bad decision ' +
                    decision,
            );
        }
        ...
        decision = HARD_TOTAL_DECISION[softCount].get(
            dealerTopCard.rank,
        ) as string;
        if (!isDecisionValid(decision)) {
            throw new Error(
                'determineBasicStrategyPlay() HARD_TOTAL_DECISION returns a bad decision ' +
                    decision,
            );
        }
    ...
    console.log(
        'determineBasicStrategyPlay() ran into a little trouble in town.',
    );
    throw new Error(
        'determineBasicStrategyPlay() ran into a little trouble in town.',
    );
