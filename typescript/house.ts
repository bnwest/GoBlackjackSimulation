export class HouseRules {
    ...
    static DECKS_IN_SHOE: number = 6
    static FORCE_RESHUFFLE: number = Math.floor(((52 * HouseRules.DECKS_IN_SHOE) * 3) / 4)
    ...
    static DOUBLE_DOWN_AFTER_SPLIT: boolean = true

    // 3 => turn one hand into no more than 4 hands
    static SPLITS_PER_HAND: number = 3

    // rank match like K-K always can split, values match allows K-10 split
    static SPLIT_ON_VALUE_MATCH: boolean = true

    // Hit on soft 17 (6/8 decks) is more common on low bet tables.
    static DEALER_HITS_HARD_ON: number = 16  // or less
    static DEALER_HITS_SOFT_ON: number = 17  // or less

    // 1.5 => 3 to 2 payout, 1.2 => 6 to 5 payout
    // 6 to 5 is more common in two deck games
    static NATURAL_BLACKJACK_PAYOUT: number = 1.5
    ...
    static SURRENDER_ALLOWED: boolean = true
}
