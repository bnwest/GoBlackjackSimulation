...
class PlayerMasterHand {
  ...
  splitHand(handIndex: number, cardsToAdd: Card[]) {
    ...
    const newPlayerHand: PlayerHand = new PlayerHand(oldPlayerHand.bet, true);
  ...
  playGame() {
    ...
    if (dealer.hand.isNatural) {
          ...
          for (let k = 0; k < masterHand.numHands; k++) {
            ...
            while (true) {
              ...
              if (playerDecision === PlayerDecision.STAND) {
                ...
                const newHandIndex: number = masterHand.splitHand(
                  handIndex,
                  cardsToAdd,
                );
                log(
                ...
                const splittingAces: boolean = card1.rank === CardRank.ACE;
                if (
                  splittingAces &&
      ...
      while (!dealerDone) {
        ...
        const useSoftCount: boolean = hardCount < softCount && softCount <= 21;
            ...
            if (hand.outcome === HandOutcome.BUST) {
              ...
              if (hand.isNatural) {
                const payout: number = Math.floor(
                  hand.bet * HouseRules.NATURAL_BLACKJACK_PAYOUT,
                );
